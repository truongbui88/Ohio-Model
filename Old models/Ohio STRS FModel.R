...
RunModel <- function(DR_CurrentHires = dis_r_proj_currentHires,
  ...
  for(i in StartIndex:length(FYE)){
    ...
    TotalPayroll[i] <- payroll
    
    CurrentHirePayroll[i] <- payroll_DB_Legacy
    NewHirePayroll[i] <- payroll_DB_Future
    ...
    AccrLiabOrigDR_CurrentHires[i] <- (PVFB_DB_Legacy + PVFB_CB_Legacy) - (PVFNC_DB_Legacy + PVFNC_CB_Legacy)
    AccrLiabOrigDR_NewHires[i] <- (PVFB_DB_Future + PVFB_CB_Future) - (PVFNC_DB_Future + PVFNC_CB_Future)
    ...
    Total_NC_CurrentHires_Pct[i] <- nc_rate_DB*0.95
    Total_NC_NewHires_Pct[i] <- nc_rate_DB[EntryYear >= 2010]
    Total_NC_DB_CurrentHires_Pct[i] <- nc_rate_DB*0.05
