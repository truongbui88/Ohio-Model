...
MortalityTable <- MortalityTable_int %>% 
  ...
  mutate(RetirementCond = IsRetirementEligible(Age, YOS, RetYear),
         ...
         mort_male = ifelse(RetirementCond == F, Pub2010_employee_male, #Adding adj. facctors
                            Pub2010_healthy_retiree_male * 1.1) * MPcumprod_male_adj,
         mort_female = ifelse(RetirementCond == F, Pub2010_employee_female * 0.95,
...
MortalityTable_retire <- expand_grid(Age = Age[Age >= 40], Years = Years[Years >= YearStart]) %>% 
  ...
  mutate(base_age = Age - (Years - YearStart),
         mort_male = Pub2010_healthy_retiree_male * MPcumprod_male_adj,
         mort_female = Pub2010_healthy_retiree_female * MPcumprod_female_adj,
         mort = (mort_male + mort_female)/2) %>% 
...
SeparationRates <- SeparationRates %>% 
  mutate(retirement_type = IsRetirementEligible_Regular(Age,YOS, RetYear),
         SepRateMale_DB = ifelse(retirement_type == T, DBMale_Ret_Unred, DBMale_Ret_Red),
         SepRateFemale_DB = ifelse(retirement_type == T, DBFemale_Ret_Unred, DBFemale_Ret_Red),
...
SeparationRates <- SeparationRates %>% select(EntryYear, entry_age, RetYear, Age, YOS, RemainingProb_DB, SepProb_DB, SepRate_DB)
